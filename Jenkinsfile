...
static final String REPO = "react-picture-annotation"
  ...
  volumes: [
    ...
    container('node') {
      ...
      stage('Deploy storybook') {
        ...
        sh("cognite-release-manager deploy-storybook -z storybook.tar.gz --output=storybook.md --repo react-picture-annotation")
      }
      ...
      if (env.BRANCH_NAME == 'master') {
        stage('Publish to npm') {
          sh("yarn add --dev @cognite/release-manager")
          sh('cognite-release-manager publish-module')
        }
      }
    }
  }

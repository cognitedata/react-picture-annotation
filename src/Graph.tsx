...
const getMidPoint = (x: number = 0, y: number = 0) => (x + y) / 2;

// TODO (future): Draw 1 line instead of 2
const getLinkId = (link: SimulationLinkDatum<Node>) => {
...
export type GetOffsetFunction<T> = (
  link: SimulationLinkDatum<Node & T>
) => {
...
export const Graph = <T,>({
  ...
  const addFixedNode = {
    mutate: (_nodeDetails: { id: number; x: number; y: number }) => {
      // noop
    },
  };
  ...
  const links: SimulationLinkDatum<Node>[] = useMemo(() => {
    // make lookups easy
  ...
  }, [d3Nodes, simulation, transform, addFixedNode, useFixedNodes]);

  useImperativeHandle(ref, () => ({
    zoomIn: (scaleFactor: number = 1.1) => {
    ...
    zoomOut: (scaleFactor: number = 1.1) => {
      const newScaleK = transform.k / scaleFactor;
      onZoom({
        ...transform,
